window.addEventListener("load", () => {
    fetch('http://127.0.0.1:8080/bookauthors', { method:"GET"})
    ...
    function updateAuthorList(authors){
        ...
        for(i=0; i<btns_izmeni.length; i++){
            btns_izmeni[i].addEventListener("click", function(){
                ...
                const inputs = prompt("Upisite book_id,author_id u ovom formatu (ostavite prazno ako ne menjate)", "").split(",");
                if (inputs == null) {
                    ...
                    fetch("http://127.0.0.1:8080/bookauthors/" + this.parentNode.parentNode.dataset.authorid, {
                        method:"PUT",
                        headers: {
        ...
        for(i=0; i<btns_obrisi.length; i++){
            btns_obrisi[i].addEventListener("click", function(){
                ...
                fetch("http://127.0.0.1:8080/bookauthors/obrisi/" + this.parentNode.parentNode.dataset.authorid,
                    { method:"DELETE" })
                    .then( response=>response.json())
                    .then( data => {
                        fetch('http://127.0.0.1:8080/bookauthors', { method:"GET" })
        ...
        document.getElementById("btn_filter").addEventListener("click", function(){
            ...
            fetch("http://127.0.0.1:8080/bookauthors/"+q, { method:"GET" })
                .then( response=>response.json())
                .then( data =>  updateAuthorList(data) );
        });
    ...
    document.getElementById("btn_dodaj").addEventListener("click", function(){
        let noviAutor = {
            ...
            author_id: document.getElementById("novi_autor_author_id").value
        };

        nt = JSON.stringify(noviAutor);
        fetch("http://127.0.0.1:8080/bookauthors/dodaj", {
            method:"POST",
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
        ...
        document.getElementById("novi_autor_author_id").value = '';
    });
})