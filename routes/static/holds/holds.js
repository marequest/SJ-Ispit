window.addEventListener("load", () => {
    fetch('http://127.0.0.1:8080/holds', { method:"GET"})
    ...
    function updateAuthorList(authors){
        ...
        for(i=0; i<btns_izmeni.length; i++){
            btns_izmeni[i].addEventListener("click", function(){
                ...
                const inputs = prompt("Upisite start_time,end_time,book_id,patron_id u ovom formatu (ostavite prazno ako ne menjate)", "").split(",");
                if (inputs == null) {
                    ...
                    fetch("http://127.0.0.1:8080/holds/" + this.parentNode.parentNode.dataset.authorid, {
                        method:"PUT",
                        headers: {
        ...
        for(i=0; i<btns_obrisi.length; i++){
            btns_obrisi[i].addEventListener("click", function(){
                ...
                fetch("http://127.0.0.1:8080/holds/obrisi/" + this.parentNode.parentNode.dataset.authorid,
                    { method:"DELETE" })
                    .then( response=>response.json())
                    .then( data => {
                        fetch('http://127.0.0.1:8080/holds', { method:"GET" })
        ...
        document.getElementById("btn_filter").addEventListener("click", function(){
            ...
            fetch("http://127.0.0.1:8080/holds/"+q, { method:"GET" })
                .then( response=>response.json())
                .then( data =>  updateAuthorList(data) );
        });
    ...
    document.getElementById("btn_dodaj").addEventListener("click", function(){
        ...
        fetch("http://127.0.0.1:8080/holds/dodaj", {
            method:"POST",
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
